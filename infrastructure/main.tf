...
resource "aws_s3_bucket" "frontend" {
  bucket = "coauthor-frontend-farhan"
}
...
resource "aws_cloudfront_distribution" "frontend" {
  ...
  # SPA fallback: serve index.html for any missing path (client-side routing)
...
resource "aws_ecs_cluster" "main" {
  name = "coauthor-cluster"
}

resource "aws_iam_role" "ecs_task_execution" {
  name = "coauthor-ecs-task-execution-role"
...
# Task role: grants the running container permission to call AWS services
resource "aws_iam_role" "ecs_task" {
  name = "coauthor-ecs-task-role"
...
resource "aws_iam_role_policy" "ecs_task_aws_access" {
  name = "coauthor-ecs-task-aws-access"
    ...
    Statement = [
      {
        ...
        Resource = aws_lambda_function.scan_document.arn
      },
      {
        Effect = "Allow"
...
resource "aws_cloudwatch_log_group" "backend" {
  name              = "/ecs/coauthor-backend"
  retention_in_days = 7
}

resource "aws_ecs_task_definition" "backend" {
  family                   = "coauthor-backend"
  ...
  container_definitions = jsonencode([{
    ...
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        "awslogs-group"         = "/ecs/coauthor-backend"
